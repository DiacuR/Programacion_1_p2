...
void mostrarListadoAlumnos(eAlumno listadoDeAlumnos[], int tamAlu,eLocalidad listadoDeLocalidades[],int tamLoc)
{
    int i,j;
    int index;
    eLocalidad localidadAlumno;
    ...
    for(i=0; i<tamAlu; i++)
    {
        if(listadoDeAlumnos[i].estado==OCUPADO)
        {
            for(j = 0; i < tamLoc; i++)
            {
                if(listadoDeAlumnos[i].idLocalidad == listadoDeLocalidades[j].idLocalidad)
                {
                    mostrarUnAlumno(listadoDeAlumnos[i],listadoDeLocalidades[j]);
                }
            }
...
void buscarLocalidad(eLocalidad[],int,int)
{
    int i;
    int j;
}
