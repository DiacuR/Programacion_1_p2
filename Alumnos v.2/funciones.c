...
void inicializarAlumnos(eAlumno listaDeAlumnos[],int cantidad)
{
    int i;
    for(i=0; i<cantidad; i++)
    {
        listaDeAlumnos[i].estaVacio=LIBRE;
    }
}
...
void harcodearMaterias(eMaterias listado[],int tam)
{
    ...
    char descripcion [][30] = {"Programacion I","Laboratorio I","Matematica I","SPD"};
    int cargaHoraria [] = {4,4,6,4};


    for(i = 0; i < tam; i++)
    {
        listado[i].idMateria = id[i];
        strcpy(listado[i].descripcion, descripcion[i]);
        listado[i].cargaHoraria = cargaHoraria[i];
    }
}
...
void mostrarMaterias(eMaterias listado[], int tam)
{
    ...
    for(i = 0; i < tam; i++)
    {
        printf("\n%d\t%s\n", listado[i].idMateria, listado[i].descripcion);
    }
}
...
{
    int index;
    eInscripcion inscripcionAlum;



    index = dameIndiceInscripcion(listadoDeInscripcion,tamInsc);

    if(index != -1)
    {
        listarAlumnosConLocalidad(listadoDeAlum,tamAlu,listLocalidad,tamLoc);
        printf("Ingrese legajo del alumno: ");
        scanf("%d",&inscripcionAlum.legajo);

        mostrarMaterias(listadoDeMaterias,tamMat);
        printf("Ingresar id materia: ");
        scanf("%d", &inscripcionAlum.idMateria);

        printf("Ingresar fecha:");
        scanf("%d/%d/%d", &inscripcionAlum.fechaDeInscripcion.dia, &inscripcionAlum.fechaDeInscripcion.mes, &inscripcionAlum.fechaDeInscripcion.anio);
    }
}
