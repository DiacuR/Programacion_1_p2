#include "Alumno.h"
/*
    al mostrar alum. en lugar de mostrar idlocalidad demuestre la descripcion
*/
...
int buscarAlumnoPorLegajo(eAlumno listado[],int tam)
{
    int i, legajo;
    int indice = -1;

    mostrarListadoAlumnos(listado,tam);
...
int modificarAlumno(eAlumno listado[],int tam)
{
    ...
    printf("Desea cambiar el promedio: ");
    respuesta = getchar();
    if(respuesta == 's')
    {
    ...
    index = buscarAlumnoPorLegajo(listado,tam);
